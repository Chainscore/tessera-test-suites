...
def apply_discriminator(
    ...
    Raises:
        TypeError:
            ...
            - If `discriminator` value mapped to multiple composite.
...
class _ApplyInferredDiscriminator:
    ...
    * Introspecting the schema to determine which discriminator values should map to which union composite
    * Handling various edge cases such as 'definitions', 'default', 'nullable' schemas, and more
    ...
    def __init__(self, discriminator: str, definitions: dict[str, core_schema.CoreSchema]):
        ...
        # This may change if/when we expose a way to manually specify the TaggedUnionSchema's composite.
        ...
        # for different composite. This is not a limitation of pydantic_core, but if we try to handle
        ...
        # as an indication that, even if None is discovered as one of the union composite, we will not need to wrap
        ...
        # `_choices_to_handle` serves as a stack of composite to add to the tagged union. Initially, composite
        # from the union in the wrapped schema will be appended to this list, and the recursive choice-handling
        # algorithm may add more composite to this stack as (nested) unions are encountered.
        self._choices_to_handle: list[core_schema.CoreSchema] = []

        # `_tagged_union_choices` is built during the call to `apply`, and will hold the composite to be included
    ...
    def _apply_to_root(self, schema: core_schema.CoreSchema) -> core_schema.CoreSchema:
        ...
        method iteratively on the composite extracted (recursively) from the possibly-wrapped union.
        """
        ...
        # Reverse the composite list before extending the stack so that they get handled in the order they occur
        choices_schemas = [v[0] if isinstance(v, tuple) else v for v in schema['composite'][::-1]]
    ...
    def _handle_choice(self, choice: core_schema.CoreSchema) -> None:
        ...
        (and any "coalesced" composite obtained from inner unions).

        Here, "handling" entails:
        * Coalescing nested unions and compatible tagged-unions
        * Tracking the presence of 'none' and 'nullable' schemas occurring as composite
        ...
        * Updating the _tagged_union_choices mapping that will ultimately be used to build the TaggedUnionSchema.
        ...
        if choice['type'] == 'none':
            ...
            # Reverse the composite list before extending the stack so that they get handled in the order they occur
            choices_schemas = [v[0] if isinstance(v, tuple) else v for v in choice['composite'][::-1]]
            self._choices_to_handle.extend(choices_schemas)
            ...
            if choice['type'] == 'tagged-union' and self._is_discriminator_shared(choice):
                ...
                # and its composite can be coalesced into the outer TaggedUnionSchema.
                subchoices = [x for x in choice['composite'].values() if not isinstance(x, (str, int))]
                # Reverse the composite list before extending the stack so that they get handled in the order they occur
    ...
    def _infer_discriminator_values_for_choice(  # noqa C901
        ...
        if choice['type'] == 'definitions':
            ...
            values: list[str | int] = []
            # Ignore str/int "composite" since these are just references to other composite
            subchoices = [x for x in choice['composite'].values() if not isinstance(x, (str, int))]
            ...
            values = []
            for subchoice in choice['composite']:
    ...
    def _infer_discriminator_values_for_inner_schema(
        ...
        if schema['type'] == 'literal':
            ...
            values: list[Any] = []
            for choice in schema['composite']:
    ...
    def _set_unique_choice_for_values(self, choice: core_schema.CoreSchema, values: Sequence[str | int]) -> None:
        ...
        for discriminator_value in values:
            if discriminator_value in self._tagged_union_choices:
                ...
                # Because tagged_union_choices may map values to other values, we need to walk the composite dict
                ...
                if existing_choice != choice:
                    raise TypeError(
                        ...
                        f'{self.discriminator!r} mapped to multiple composite'
                    )
            else:
