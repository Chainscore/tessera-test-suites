...
def test_zero_refine(db_path):
    service = "zero"
    vm_handle = 0          # child VM handle if you created one via machine(...);
                           # some environments may accept 0 for "current".
    page = 0               # page index (not bytes)
    count = 1              # number of pages to zero
    payload = struct.pack("<QQQ", vm_handle, page, count)
    ...
    wi_service_code = open(Path(__file__).parents[3] / "tessera-test-suites" / "playground" / "builds" / f"{service}-service.jam", "rb").read()
