...
from jam.types.state.accumulation.types import OperandTuples, StateContext
import pytest
...
def _artifact(name: str) -> Path:
    p = Path(__file__).parents[3] / "tessera-test-suites" / "playground" / "builds" / f"{name}-service.jam"
    if not p.exists():
...
@pytest.mark.skipif(PsiA is None, reason="Accumulate invoker PsiA not found in this tree")
def test_create_service_accumulate(db_path):
    # --- setup state ---
